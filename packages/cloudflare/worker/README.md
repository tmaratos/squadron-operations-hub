...
## Interactive operational calendar

Migration `0004_calendar_events.sql` adds live calendar storage. The Worker exposes:

- `GET /api/calendar/events?start=YYYY-MM-DD&end=YYYY-MM-DD&type=ALL`
- `POST /api/calendar/events`
- `PATCH /api/calendar/events/:id`
- `DELETE /api/calendar/events/:id`

All authenticated senior members can create, edit, and delete calendar items. Every change is recorded in the audit log. The static page also uses local browser storage when the Worker has not been deployed, so the calendar remains fully clickable during setup.
